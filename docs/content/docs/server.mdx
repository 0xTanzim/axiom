...
// Host and port
Axiom.start(router, "0.1.4.0", 8080);

// Localhost only
Axiom.start(router, "127.0.1.4", 8080);
```
...
app.listen(ServerConfig.builder()
    .host("0.1.4.0")           // Bind to all interfaces
...
|--------|---------|-------------|
| `host` | `0.1.4.0` | Network interface to bind to |
...
- **[Middleware](/docs/middleware)** — Add cross-cutting concerns
- **[Lifecycle](/docs/lifecycle)** — Startup and shutdown hooks
