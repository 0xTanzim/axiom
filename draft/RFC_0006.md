...
**Status:** Draft (Updated 2026-01-18)
...
## 2. Core Principle (Updated)

> The framework is build-tool agnostic and language-friendly by design.

- No build tool assumptions
- No required plugins
- **Compile-time annotation processing ALLOWED for DI only** (see Section 2.1)
- No Kotlin-first bias
- No Java-second-class treatment

### 2.1 Annotation Processing Exception (DI Only)

**Amendment (2026-01-18)**: Compile-time annotation processing is **permitted** for:

- Dependency Injection code generation (`axiom-di` module)
- `@Service`, `@Repository`, `@Routes`, `@Inject` processing
- Dagger 2 integration

**Rationale**: DX improvement outweighs the cost. Manual wiring creates developer friction.

**Constraints**:
- Generated code must be **readable and debuggable**
- No runtime reflection for DI
- No classpath scanning
- Users can opt-out by not using `axiom-di`

**NOT permitted**:
- Build plugins that modify user code
- Runtime bytecode enhancement
- Classpath scanning for auto-discovery
- Magic behavior that users cannot trace
...
```
axiom-core.jar
axiom-di.jar (with annotation processor)
axiom-server.jar
(optional adapters)
```
